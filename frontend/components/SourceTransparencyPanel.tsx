...
export function SourceTransparencyPanel({ detail }: { detail: RetrievalDetail[] }) {
  if (detail.length === 0) {
    return <p className="py-2 text-sm text-zinc-500 dark:text-zinc-400">No retrieval detail available for this answer.</p>;
  }
    ...
    <div className="space-y-4 py-1">
      {detail.map((subtopic) => (
        <div key={subtopic.label}>
          <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{subtopic.label}</p>
          ...
          <div className="mb-2 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-zinc-500 dark:text-zinc-400">
            {Object.entries(subtopic.candidates_by_source).map(([source, count]) => (
              <span key={source}>
                {SOURCE_LABELS[source] ?? source}: <span className="font-medium text-zinc-700 dark:text-zinc-300">{count}</span>
              </span>
            ))}
          </div>
          {subtopic.selected.length === 0 ? (
            <p className="text-xs text-zinc-400 dark:text-zinc-500">No chunks were relevant enough to select.</p>
          ) : (
              ...
              {subtopic.selected.map((chunk, i) => (
                ...
                >
                  ...
                  <span className="flex-1">
                    {chunk.link ? (
                      <a href={chunk.link} target="_blank" rel="noopener noreferrer" className="underline hover:no-underline">
                        {chunk.title}
                      </a>
                    ) : (
                      chunk.title
                    )}{" "}
                    <span className="text-zinc-400 dark:text-zinc-500">({chunk.section})</span>
                    {chunk.cited && (
